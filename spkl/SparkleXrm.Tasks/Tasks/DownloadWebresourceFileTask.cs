...
namespace SparkleXrm.Tasks
{
    public class DownloadWebresourceFileTask : BaseTask
    {
        ...
        protected override void ExecuteInternal(string filePath, OrganizationServiceContext ctx)
        {
            ...
            {
                throw new SparkleTaskException(SparkleTaskException.ExceptionTypes.CONFIG_NOTFOUND,
                    $"spkl.json not found at '{filePath}':{ex.Message}");
            }
            ...
            var existingWebResources = new Dictionary<string, WebResourceFile>();
            foreach (var file in files)
            {
                var key = file.uniquename.ToLower();
                if (!existingWebResources.ContainsKey(key))
                    existingWebResources[key] = file;
            }

            var webresourceConfig = config.GetWebresourceConfig(Profile).FirstOrDefault();
            ...
            var solutions = config.GetSolutionConfig(Profile);
            if (solutions == null || solutions.Length == 0)
                throw new SparkleTaskException(SparkleTaskException.ExceptionTypes.CONFIG_NOTFOUND,
                    "Solution section not found in spkl.json. This is needed to determine where to store the webresources!");

            var rootPath = Path.Combine(config.filePath, webresourceConfig.root != null ? webresourceConfig.root : "");

            foreach (var solution in solutions)
            {
                var downloadedWebresources =
                    ServiceLocator.Queries.GetWebresourcesInSolution(_context, solution.solution_uniquename);

                if (downloadedWebresources.Count == 0)
                    throw new SparkleTaskException(SparkleTaskException.ExceptionTypes.NO_WEBRESOURCES_FOUND,
                        $"No webresources found to download in the solution '{solution.solution_uniquename}'");

                var maps = solution.map.Where(e => e.from.StartsWith("WebResources"));
                if (maps == null || maps.Count() == 0)
                    throw new SparkleTaskException(SparkleTaskException.ExceptionTypes.CONFIG_NOTFOUND,
                        $"No maps section in the solution packager config. This is needed to determine where to store the webresources!");

                var webresourceRootFolder = string.IsNullOrEmpty(webresourceConfig.root)
                    ? filePath
                    : Path.Combine(filePath, webresourceConfig.root);
                foreach (var resource in downloadedWebresources)
                {
                    ...
                    {
                        //var from = RemoveTrailingFolderSeperator(map.from);
                        var fromfolder = Path.GetDirectoryName(map.from);
                        var fromfile = Path.GetFileName(map.from);

                        var to = Path.GetFullPath(Path.Combine(webresourceRootFolder,
                            RemoveTrailingFolderSeperator(map.to)));
                        ...
                        if (map.map == MapTypes.path &&
                            name.StartsWith(fromfolder, StringComparison.InvariantCultureIgnoreCase)
        ...
        public bool Wildcard(string pattern, string input)
        {
        ...
        private string RemoveTrailingFolderSeperator(string folder)
        {
            ...
            while (folder[folder.Length - 1] == Path.DirectorySeparatorChar ||
                   folder[folder.Length - 1] == Path.AltDirectorySeparatorChar)
